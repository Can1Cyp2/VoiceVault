...
export async function playNoteSound(noteName: string): Promise<Audio.Sound | null> {
  const fileKey = noteToFileKey(noteName);
  ...
  try {
    ...
    console.warn('Error playing note:', noteName, error);
    return null;
  }
}
